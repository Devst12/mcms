  ...
  return (
    <div className="p-4 space-y-4 pb-8">
      ...
      <div className="space-y-1">
        <h1 className="text-3xl-large font-bold">नमस्ते, तिलक नारायण श्रेष्ठ जी 🙏</h1>
        <p className="text-2xl-large font-bold tabular-nums">{formatBsDateNepali(todayBs)}</p>
      ...
      <Link
        href="/entry"
        className="flex items-center justify-center gap-2 w-full py-4 min-h-touch bg-blue-600 text-white rounded-xl font-bold text-lg shadow-[3px_3px_0_rgba(0,0,0,0.2)] active:shadow-none active:translate-x-[3px] active:translate-y-[3px] transition-all"
      >
      ...
      {hasData ? (
        <>
          ...
          <div className="card space-y-3">
            <h2 className="text-xl-large font-bold">आजको संकलन</h2>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-cow border-2 border-gray-800 rounded-xl p-4 shadow-[3px_3px_0_rgba(0,0,0,0.15)]">
                ...
                <p className="text-4xl-large font-bold tabular-nums">{cowTotal.toFixed(1)}</p>
                <p className="text-large text-gray-600">लिटर</p>
                {companyCow > 0 && (
                  <div className="mt-2 flex items-center gap-2 text-large">
                    {cowDiff === 0 ? (
              ...
              <div className="bg-buffalo border-2 border-gray-800 rounded-xl p-4 shadow-[3px_3px_0_rgba(0,0,0,0.15)]">
                ...
                <p className="text-4xl-large font-bold tabular-nums">{buffaloTotal.toFixed(1)}</p>
                <p className="text-large text-gray-600">लिटर</p>
                {companyBuffalo > 0 && (
                  <div className="mt-2 flex items-center gap-2 text-large">
                    {buffaloDiff === 0 ? (
            ...
            <div className="flex items-center justify-between bg-gray-50 border-2 border-gray-300 rounded-xl px-4 py-3">
              ...
              <span className="text-2xl-large font-bold tabular-nums">{enteredCount}/{totalFarmers}</span>
            </div>
          </div>
          ...
          <div className="card">
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-3">
                <p className="text-3xl-large font-bold tabular-nums text-blue-700">{totalFarmers}</p>
                <p className="text-large font-bold text-gray-600">किसान</p>
              </div>
              <div className="bg-green-50 border-2 border-green-200 rounded-xl p-3">
                <p className="text-3xl-large font-bold tabular-nums text-green-700">{(cowTotal + buffaloTotal).toFixed(1)}</p>
                <p className="text-large font-bold text-gray-600">जम्मा लिटर</p>
              </div>
              <div className="bg-purple-50 border-2 border-purple-200 rounded-xl p-3">
                <p className="text-3xl-large font-bold tabular-nums text-purple-700">{enteredCount}</p>
                <p className="text-large font-bold text-gray-600">भयो</p>
              </div>
            </div>
          </div>
          ...
          {notEntered.length > 0 && (
            <div className="card border-amber-400 bg-amber-50">
              <h2 className="text-xl-large font-bold text-amber-800 mb-3">आज दूध थपिएको छैन</h2>
              <div className="space-y-2">
                {notEntered.slice(0, 10).map((f) => (
                  <div key={f._id} className="flex items-center justify-between bg-white border-2 border-amber-300 rounded-xl px-4 py-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{f.code}</p>
                    </div>
                    <Link
                      href={`/entry?farmerId=${f._id}`}
                      className="px-5 py-3 min-h-touch bg-blue-600 text-white rounded-lg text-large font-bold shadow-[2px_2px_0_rgba(0,0,0,0.2)] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] transition-all"
                    >
                      थप्नुहोस्
                    </Link>
                  </div>
                ...
                {notEntered.length > 10 && (
                  <Link href="/entries" className="block text-center text-large font-bold text-blue-700 py-2">
                    थप हेर्नुहोस्
                  </Link>
                )}
              </div>
          ...
          {(companyCow > 0 || companyBuffalo > 0) && (
            <div className="card">
              <h2 className="text-large font-bold mb-3">कम्पनी तुलना</h2>
              <div className="space-y-2">
        ...
        <div className="card text-center py-8">
          ...
          <h2 className="text-2xl-large font-bold mb-2">आजको दूध अझै थपिएको छैन</h2>
          <p className="text-large text-gray-600 mb-4">पहिलो प्रविष्टि थप्नुहोस्।</p>
          <Link
            href="/entry"
            className="inline-flex items-center justify-center gap-2 w-full py-4 min-h-touch bg-blue-600 text-white rounded-xl font-bold text-lg shadow-[3px_3px_0_rgba(0,0,0,0.2)] active:shadow-none active:translate-x-[3px] active:translate-y-[3px] transition-all"
          >
            <span className="text-2xl">➕</span>
            दूध थप्नुहोस्
          </Link>
