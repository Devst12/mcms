...
export async function getRateSlabs(milkType?: "cow" | "buffalo"): Promise<RateSlab[]> {
  const db = await getDb();
  const query = milkType ? { milkType } : {};
  return db.collection<RateSlab>("rate_slabs").find(query).sort({ effectiveFromAD: -1 }).toArray();
}

export async function getActiveRateSlab(milkType: "cow" | "buffalo", dateAD: string): Promise<RateSlab | null> {
  const db = await getDb();
  return db
